...
async function fetchMyCourses() {
    ...
    try {
        ...
        if(grid) grid.innerHTML = `<p class="text-red-500 col-span-full text-center py-10 italic">Erro ao sincronizar biblioteca. Verifique sua conexão.</p>`;
    }
}
...
function renderEmptyState(container) {
    ...
    container.innerHTML = `
        <div class="animate-fade-in-down max-w-lg">
            ...
            <p class="text-gray-500 text-sm mb-12 px-6 italic">Você ainda não possui protocolos ativos.</p>
            <a href="/aluno/catalogo.html" class="inline-block px-14 py-5 bg-gold text-black font-black uppercase tracking-[0.3em] text-[10px] rounded-full">Acessar Coleção 2026</a>
        </div>`;
}
...
function renderLibrary(list) {
    ...
    grid.innerHTML = list.map(item => {
        ...
        return `
        <div class="group bg-[#1a1a1a] border border-white/5 rounded-sm overflow-hidden flex flex-col">
            <div class="relative h-40">
                <img src="${course.imageUrl || 'https://via.placeholder.com/400x225'}" class="w-full h-full object-cover opacity-80 group-hover:opacity-100">
                <a href="/assistir.html?id=${course.id}" class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
            ...
            <div class="p-5 flex flex-col flex-grow">
                <span class="text-[9px] text-gold uppercase mb-2 font-bold">${course.category || 'MÓDULO'}</span>
                ...
                <div class="mt-auto">
                    ...
                    <p class="text-[8px] text-gray-500 text-right">${progress}%</p>
                </div>
            </div>
        </div>`;
    }).join("");
